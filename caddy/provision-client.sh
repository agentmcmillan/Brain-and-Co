...
usage() {
    ...
    echo ""
    echo "Examples:"
    echo "  $0 conor-laptop 'Conor Laptop'"
    echo "  $0 spark-device-1 'Spark Device 1' --deploy ubuntu@10.0.0.100:~/.mcp-certs"
    echo ""
    echo "The script will:"
    echo "  1. Generate a 2048-bit RSA key pair"
    echo "  2. Create a CSR and sign it with the Network MCP CA"
    echo "  3. Bundle into a .p12 for easy import"
    echo "  4. Optionally deploy via SCP to a remote host"
    echo "  5. Print connection config for Claude Code"
    exit 1
}
...
openssl req -new \
    ...
    -subj "/C=US/O=Cubic Build/OU=Network MCP/CN=${CN}"
...
echo "Add to ~/.claude.json mcpServers (or project .mcp.json):"
echo ""
cat << EOF
{
  "mcpServers": {
